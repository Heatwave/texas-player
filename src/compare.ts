const HAND_RANK_LIST = require('../data/hand-rank.json');


export function getHandRank(hand: Array<string>): number {
    /**
     * 707: ['9D', '7C'] 8.0% 2.5%
     */
    let rank;
    for (rank = 0; rank < HAND_RANK_LIST.length; rank++) {
        const item = HAND_RANK_LIST[rank];
        if (item.indexOf(hand[0].toUpperCase()) > -1 && item.indexOf(hand[1].toUpperCase()) > -1)
            break;
    }
    return rank;
}
...
export function rankHandInt(hand) {
    ...
    /*
     * hand.card[i] are the series of all 7 cards (2 private + 5 public)
     * for example:
     *
     * hand.cards[0] = QH
     * hand.cards[1] = 6C
